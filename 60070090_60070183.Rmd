...
***
### ชื่อ:    อลิส แดนอัศจรรย์ 

### รหัสนักศึกษา:   59xxxxxx

### เวลาเรียน: อังคาร 9.00-12.00 
***

## โจทย์ปัญหา  :
### 1. ไทยเป็นประเทศขี้เมาอันดับต้นๆของโลกจริงหรือไม่? 
### 2. ประเทศใดมีการบริโภคเครื่องดื่มแอลกอฮอล์สูงที่สุดในโลก?

### ชุดข้อมูลที่ใช้จาก  fivethirtyeight : drinks
...
# make it tidy (this code is taken from the example given in ?drinks)
drinks_tidy <- drinks %>%
...
```{r}
country25 <- drinks_tidy %>%
              # grouped by country
              group_by(country) %>% 
              # sum of servings in each country
              summarise(total_servings = sum(servings)) %>%
              # sort rows by total servings consumed
              arrange(desc(total_servings)) %>%
              # select only the top 25 countries (first 25 items on the list)
              head(25) 
...
```{r}
p1 <- ggplot(data=top25_and_th, aes(x=reorder(country, servings),y=servings)) +
        # map "type" of beverage to aes "fill"" of the bar 
        geom_col(aes(fill=type)) + 
        # flip coordinate 
        coord_flip() +
        # add labels
        labs(x="", y="Consumption (servings)", title="Top 25 alcohol consuming countries \nand Thailand")
